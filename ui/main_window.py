...
class MainWindow(QMainWindow):
    ...
    def open_category_stats(self):
        """Kategori İstatistikleri menüsünü aç"""
        try:
            from .category_stats_window import CategoryStatsWindow
            stats_window = CategoryStatsWindow(self.colors, self.return_to_main)
            self.stacked_widget.addWidget(stats_window)
            self.stacked_widget.setCurrentWidget(stats_window)
        except Exception as e:
            self.show_error(f"Kategori İstatistikleri açılırken hata: {str(e)}")
